...
const Warning = ({ message , handleConfirmWarning, handleCloseWarning}) => {
    return (
        <div className='fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 backdrop-blur-sm z-50'>
            <div className=' h-40 bg-white w-80 rounded-lg  shadow-xl'>
                ...
                <div className='flex space-x-3 justify-end px-5 font-bold text-sm font-mono'>
                    <button onClick={() => handleConfirmWarning()} className='text-blue-700 hover:text-blue-600'>Confirm</button>
                    <button onClick={() => handleCloseWarning()} className='text-red-700 hover:text-red-600'>Close</button>
                </div>
            </div>
        </div>
    )
